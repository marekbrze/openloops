...
- **Karta wątku**: tytuł (statyczny — klik zaznacza, ADR-0029), żaba wyróżniona całą kartą — zielony glif + tinta tła `bg-success/10` i hairline `border-success/40` (ADR-0037), licznik per wątek: 🏆 zwycięstwa (tinta success tylko przy > 0) · otwarte zadania (ADR-0041) **bez pasa progresu** (ADR-0038), wskaźnik „czeka", plakietka „X po terminie" gdy dotyczy, „rozpisz kroki…" przy zerze akcji.
