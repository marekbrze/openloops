...
|------|-----------|------------|--------------|
| wątek / otwarty wątek | `Loop` | Jednostka pracy: otwarty temat roboczy z listą akcji i celem na końcu; ma ręczny priorytet. Żyje na liście po lewej. | „task", „zadanie" (koliduje z akcją), „thread" (kolizja z nitką wykonania) |
...
| akcja | `Action` | Konkretny krok do podjęcia w ramach wątku; ma typ właściciela. | „task", „todo" |
| mój ruch | `MyMove` | Typ akcji wymagający działania użytkownika — historyczny licznik pasa progresu (ADR-0038: pasek zdjęty). | „aktywne zadanie" |
| czekam na kogoś | `WaitingOn` | Typ akcji zablokowanej na osobie trzeciej; wektoryzuje zbiorczy status wątku („czeka na innych"). | „blocked" (bez wskazania komu), „delegowane" |
...
| progres | `Progress` | Pojęcie historyczne: udział wykonanych akcji „mój ruch" w wątku — wizualizacja barem zdjęta (ADR-0038); skończone akcje liczą dziennik i sekcja Zwycięstwa. | „postęp projektu" |
...
| zwycięstwo | `Win` | Jednostka bilansu motywacyjnego: skończona akcja = małe zwycięstwo, domknięty wątek = większe zwycięstwo. Porzucenie nie jest zwycięstwem. Na workbench zwijana sekcja „Zwycięstwa" (puchar, ADR-0039) gromadzi małe zwycięstwa. | „punkt", „score" |
...
| żaba | `Frog` | Odkładane przez użytkownika zadanie albo wątek (inspiracja „Eat This Frog"): zielony glyph żaby. Wątek-żaba jest **stale przypięty** na szczycie listy workbench (ADR-0041); akcja-żaba jednorazowo wskakuje na szczyt kolejki Teraz. Drag & drop układa wewnątrz grup (kolejka: dalej wygrywa po fakcie). Auto-zdjęcie po zjedzeniu (done / domknięcie / porzucenie); cofnięcie nie przywraca. Wiele żab naraz. | „priorytet", „flaga" |
